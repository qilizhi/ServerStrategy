...
var UITree = function() {
			...
			success : function(result) {
				...
				$("#tree_3").jstree({
					...
					"types" : {
						...
						"root" : {
						      "icon" : "/assets/jstree/dist/themes/default/tree-icon.png",
				...
				$.ajax({
					...
					success : function(result) {
						...
						if (result.code == "success") {
							 ...
							 * .set_id(data.node,result.data.id);
							 */
						} else {
	...
	var copy = function(url) {
		...
		$('#tree_3').on('copy_node.jstree', function(e, data) {
			 ...
			 * "parent.id":data.parent, "id" : data.node.id }, success :
			 * function(result) { // 删除成功 if (result.code == "successs") {
			 * comm.showMsg('success', '消息提示', '移动成功！');
			 * 
			 * $.jstree.reference('#tree_3')
			 * .set_id(data.node,result.data.id); } else {
			 * comm.showMsg('error', '消息提示', '移动失败！');
			 * $.jstree.reference('#tree_3').refresh(); } }
			 * 
			 * });
	...
	var loadAuthorityTreeByRoleId = function(roleId) {
		var authorityUrl ="/role/authorityTree";
		$.ajax({
			url : authorityUrl,
			type : 'get',
			dataType:"json",
	...
	var insertRole_Authority = function(roleId, authorityIds) {
		var url ='/role/insertByRoleIdAndAuthIds';
		$.ajax({
			url : url,
			type : 'post',
			dataType:"json",
